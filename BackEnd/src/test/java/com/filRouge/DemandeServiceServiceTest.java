...
class DemandeServiceServiceTest {
    ...
    void testCreateDemandeService() {
        ...
        DemandeService createdDemande = demandeServiceService.createDemandeService(1L, 1L);
    ...
    void testUpdateDemande() {
        ...
        DemandeService updatedDemande = demandeServiceService.updateDemande(1L, 1L, 1L, ValidateStatus.EN_ATTENTE);

        assertNotNull(updatedDemande);
        assertEquals(ValidateStatus.EN_ATTENTE, updatedDemande.getStatut());
    ...
    @Test
    void testUpdateDemandeNotFound() {
        when(demandeServiceRepository.findById(1L)).thenReturn(Optional.empty());

        Exception exception = assertThrows(ResourceNotFoundException.class, () -> {
            demandeServiceService.updateDemande(1L, 1L, 1L, ValidateStatus.EN_ATTENTE);
        });
